...
# ✅ ГЛАВНОЕ ИСПРАВЛЕНИЕ: БЕЗ ПАПКИ PHOTOS
def send_video_smart(chat_id, filename, caption, reply_markup=None):
    ...
    bot.send_message(chat_id, "⚠️ Видео ещё не настроено. Отправь его боту заново.", reply_markup=reply_markup)
    return None
...
def handle_schedule(message):
    bot.send_message(message.chat.id, "📅 Расписание скоро будет доступно", reply_markup=main_menu())
