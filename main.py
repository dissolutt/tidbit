...
def send_video_smart(chat_id, filename, caption, reply_markup=None):
    ...
    if filename in file_ids:
        try:
            ...
            log.warning(f"Неверный file_id {filename}: {e}")
...
def handle_schedule(message):
    bot.send_message(message.chat.id, "необходима предварительная запись на занятия!", reply_markup=main_menu())
...
def handle_rental(message):
    send_video_smart(message.chat.id, "rental.mp4", RENTAL_TEXT), reply_markup=main_menu())
...
def handle_renew(message):
    bot.send_message(message.chat.id, "для продления абонемента свяжитесь с нами: +7 951 772 98 98\n e-motion74@bk.ru", reply_markup=subscriptions_menu())

@bot.message_handler(func=lambda m: m.text and m.text.lower() == "приобрести новый абонемент")
def handle_renew(message):
    bot.send_message(message.chat.id, "для приобретения абонемента свяжитесь с нами: +7 951 772 98 98\n e-motion74@bk.ru", reply_markup=subscriptions_menu())

@bot.message_handler(func=lambda m: m.text and m.text.lower() == "разовое занятие")
def handle_renew(message):
    bot.send_message(message.chat.id, "для оплаты разового занятия свяжитесь с нами: +7 951 772 98 98\n e-motion74@bk.ru", reply_markup=subscriptions_menu())

@bot.message_handler(func=lambda m: m.text and m.text.lower() == "пробное занятие")
def handle_renew(message):
    bot.send_message(message.chat.id, "для записи на пробное занятие свяжитесь с нами: +7 951 772 98 98\n e-motion74@bk.ru", reply_markup=subscriptions_menu())
